...
constexpr uint32_t MAX_FRAMES_IN_FLIGHT = 2; // Better understand
...
static constexpr void vkCheck(VkResult result, const std::string &errorMessage)
{
...
struct Vertex
{
    ...
    static VkVertexInputBindingDescription getBindingDescription()
    {
        ...
        // glVertexAttribDivisor(0) = VK_VERTEX_INPUT_RATE_VERTEX;
        // glVertexAttribDevisor(1) = VK_VERTEX_INPUT_RATE_INSTANCE;
...
private:
    void initWindow()
    {
        ...
        glfwSetFramebufferSizeCallback(window, framebufferResizeCallback); // Handled by engine anyway
    }
    ...
    void createTextureImage()
    {
        ...
        createImage(w, h, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImage, textureImageMemory);

        transitionImageLayout(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
        copyBufferToImage(stagingBuffer, textureImage, uint32_t(w), uint32_t(h));
    ...
    void createDescriptorSets()
    {
        ...
        vkCheck(vkAllocateDescriptorSets(device, &allocInfo, descriptorSets.data()), "failed to allocate descriptor sets!");
    ...
    void createDescriptorSetLayout()
    {
        ...
        uboLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT; // Used in which shaders
        ...
        vkCheck(vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &descriptorSetLayout), "failed to create descriptor set layout!");
    }
    ...
    void createInstance()
    {
        ...
        if constexpr (enableDebugOutput)
        {
            createInfo.enabledLayerCount = uint32_t(requestedValidationLayers.size());
    ...
    void setupDebugMessenger()
    {
        ...
        vkCheck(CreateDebugUtilsMessengerEXT(instance, &createInfo, nullptr, &debugMessenger), "failed to set up debug messenger!");
    }
    ...
    void createLogicalDevice()
    {
        ...
        createInfo.enabledExtensionCount = uint32_t(requestedDeviceExtensions.size());
        ...
        vkCheck(vkCreateDevice(physicalDevice, &createInfo, nullptr, &device), "failed to create logical device!");
    ...
    {
        // Window minimization handling, Engine can handle this anyways
    ...
    {
        auto vertShaderCode = readBinary("./res/shaders/build/test.vert.spv");
        auto fragShaderCode = readBinary("./res/shaders/build/test.frag.spv");
        ...
        rasterizer.frontFace = VK_FRONT_FACE_COUNTER_CLOCKWISE; // Counter clockwise, because we flip the Y axis
    ...
    void createCommandBuffers()
    {
        ...
        vkCheck(vkAllocateCommandBuffers(device, &allocInfo, commandBuffers.data()), "failed to allocate command buffers!");
        ...
        for (size_t i = 0; i < commandBuffers.size(); ++i)
        {
            ...
            case 4:
                ...
                break;
            default:
                throw std::runtime_error("failed to determine index type");
                break;
            }
            indexType = VK_INDEX_TYPE_UINT16; // Temp
    ...
    void createSyncObjects()
    {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            vkCheck(vkCreateSemaphore(device, &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]), "failed to create semaphore for a frame!");
            vkCheck(vkCreateSemaphore(device, &semaphoreInfo, nullptr, &renderFinishedSemaphores[i]), "failed to create semaphore for a frame!");
            vkCheck(vkCreateFence(device, &fenceInfo, nullptr, &inFlightFences[i]), "failed to create fence for a frame!");
        }
    }
    ...
    void updateUniformBuffer(uint32_t currentImage) // Test function
    {
        ...
        ubo.proj = glm::perspective(glm::radians(90.0f), swapChainExtent.width / (float)swapChainExtent.height, 0.01f, 1000.0f);
