...
Font::Font(std::string_view file, uint32_t size)
	: characters(MAX_CHARACTER_COUNT), size(size), path(std::string("res/fonts/") + file.data())
{
	...
	for (size_t i = 0; i < MAX_CHARACTER_COUNT; ++i)
	{
		...
		SK_ASSERT(!result, "FreeType: Couldn't load character: {}, from file {}", i, path);
	...
	{
		FT_Error result = FT_Load_Char(face, i, FT_LOAD_RENDER);
		SK_ASSERT(!result, "FreeType: Couldn't load character: {}, from file {}, Error: {}", i, path, FT_Error_String(result));
