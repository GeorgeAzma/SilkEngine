...
Image::Image(std::string_view file, const Props& p)
	: props(p)
{
	RawImage<> raw(file);
	if(raw.channels == 3)
...
Image::Image(const std::array<std::string, 6>& files, const Props& p)
	: props(p)
{
	RawImage<> raw(std::vector<std::string>(files.cbegin(), files.cend()));
	if (raw.channels == 3)
		raw.align4();
	props.width = raw.width;
	props.height = raw.height;
...
Image::Image(const std::vector<std::string>& files, const Props& p)
	: props(p)
{
	RawImage<> raw(files);
	if (raw.channels == 3)
...
Image::Image(const std::vector<std::array<std::string, 6>>& files, const Props& p)
	: props(p)
{
	std::vector<std::string> continous_files(files.size() * 6);
