...
RawImage<uint8_t>::RawImage(std::string_view file)
{
	load(file);
}

RawImage<uint8_t>::RawImage(const std::vector<std::string>& files)
{
	load(files);
}

void RawImage<uint8_t>::load(std::string_view file)
{
	std::string path = std::string("res/images/") + file.data();

	stbi_set_flip_vertically_on_load(true);
	stbi_uc* pixel_data = stbi_load(path.c_str(), (int*)&width, (int*)&height, (int*)&channels, 0);
	SK_ASSERT(pixel_data, "Failed to load image: {}", path);
	...
	SK_TRACE("Image Loaded: {}", path);
}

void RawImage<uint8_t>::load(const std::vector<std::string>& files)
{
	...
	for (auto& path : paths)
		path = std::string("res/images/") + path.data();
...
void RawImage<uint8_t>::save(std::string_view file)
{
	stbi_write_bmp(file.data(), (int)width, (int)height, (int)channels, pixels.data());
	SK_TRACE("Image Saved: {}", file.data());
}

void RawImage<uint8_t>::savePNG(std::string_view file)
{
