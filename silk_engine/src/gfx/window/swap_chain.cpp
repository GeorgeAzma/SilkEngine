...
SwapChain::SwapChain(VkSwapchainKHR old_swap_chain)
{
	//Choose format
...
{
	if (Window::isMinimized() || (Window::getWidth() == extent.width && Window::getHeight() == extent.height))
		return;
...
void SwapChain::create(VkSwapchainKHR old_swap_chain)
{
	extent = VkExtent2D
	(
		std::clamp(Window::getWidth(), Graphics::physical_device->getSurfaceCapabilities().minImageExtent.width, Graphics::physical_device->getSurfaceCapabilities().maxImageExtent.width),
		std::clamp(Window::getHeight(), Graphics::physical_device->getSurfaceCapabilities().minImageExtent.height, Graphics::physical_device->getSurfaceCapabilities().maxImageExtent.height)
	);
	...
	for (size_t i = 0; i < images.size(); ++i)
		this->images[i] = makeShared<Image>(images[i], getFormat());
}
